...
import com.bw.coupon.util.CouponUtil;
...
public class CouponServiceImpl implements ICouponService {
    ...
    public List<Coupon> findUserCouponsByStatus(Long userId, Integer status)
        ...
        // cache中没查到
            ...
            // 查数据库
            List<Coupon> dbCoupons = couponDao.findAllByCustomerIdAndStatus(userId,
        ...
        Map<CouponStatusEnum, List<Coupon>> classifyResult = CouponUtil.classifyCoupon(preTarget);

        // 如果已过期状态不为空, 需要做延迟处理
        if(classifyResult.containsKey(CouponStatusEnum.EXPIRED)) {
            log.info("Add Expired Coupons To Cache From FindCouponsByStatus: {}, {}",
                    userId, status);
            List<Coupon> expiredCoupons = classifyResult.get(CouponStatusEnum.EXPIRED);
            // 更新Cache
        ...
        return classifyResult.get(CouponStatusEnum.USABLE);
    }
    ...
    public Coupon acquireCoupon(AcquireTemplateRequest request) throws CommonException {
        ...
        List<Coupon> userUsableCoupons = findUserCouponsByStatus(creatorId, CouponStatusEnum.USABLE.getCode());
        ...
        Coupon coupon = new Coupon(templateId, creatorId, couponCode, CouponStatusEnum.USABLE);
        ...
        redisService.addCouponToCache(creatorId,
